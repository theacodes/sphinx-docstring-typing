...
class RedocVisitor(ast.NodeVisitor):
    ...
    def visit_Name(self, node):
        ...
        self.output += ":{ref}:`{name}`".format(ref=ref, name=name)
...
def autodoc_process_docstring(app, what, name, obj, options, lines):
    ...
    for n, line in enumerate(lines):
        ...
        try:
            ...
            new_line = re.sub(
                TYPE_RE, lambda match: transform(match.group(1)), new_line
            )

            # Check for any lingering bare type hints.
            new_line = re.sub(
                BARE_TYPE_RE, lambda match: transform(match.group(1)), new_line
            )
